...
// Utility function to provide cookie options
function getCookieOptions(maxAgeInMinutes: number): CookieOptions {
  return {
    httpOnly: true,
    secure: false,
    sameSite: "lax",
...
function AuthMiddleware(client: OAuthClient) {
  try {
    ...
    providers.forEach((provider, index) => {
      ...
      router.get(callbackUrl, async (req, res) => {
        try {
          ...
          const userData = await provider.getUserInfo(response.access_token);
          ...
          return (
            res
              .cookie("access_token", accessToken, getCookieOptions(15))
              .cookie(
                "refresh_token",
                refreshToken,
                getCookieOptions(7 * 24 * 60)
              )
    ...
    router.get(`/api/auth/refresh`, (req, res) => {
      try {
        ...
        res
          .cookie("access_token", newAccessToken, getCookieOptions(15))
